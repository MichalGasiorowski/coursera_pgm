...
for j = 1:prod(phenotypeFactor.card),
	assigment = IndexToAssignment(j, phenotypeFactor.card);
	ph_var = assigment(1);
	genes = assigment(2:end);
	gene_a = allelesToGenotypes(genes(1), genes(2));
	phenotypeFactor.val(j) = alphaList(gene_a);
	if (mod(j, 2) == 0)
